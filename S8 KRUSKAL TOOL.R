...
packages <- c("mlt", "outliers", "fitdistrplus", "logspline", "Matching", "Johnson", "actuar", "Rmisc", "ggplot2", "car", "plyr", "reshape2", "coin", "gplots", "LambertW", "nortest", "pwr", "gdata", "timeDate", "compiler") ## References at end of file.
ipak(packages)
...
### fit models
mltG1 <- mlt(ctmG1, data = mydfG1dup, trace = TRUE, maxit = 10000)
mltG2 <- mlt(ctmG2, data = mydfG2dup, trace = TRUE, maxit = 10000)
mltG3 <- mlt(ctmG3, data = mydfG3dup, trace = TRUE, maxit = 10000)
...
systimeelapsed <- startsystime - finishsystime
systimeelapsed
